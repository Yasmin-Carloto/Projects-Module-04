...
function Form() {
    ...
    const postUserInDatabase = async () => {
        try{
            const response = await axios.post("https://reqres.in/api/users", formFieldsData)
            return response
        }catch(error){
            return error
        }
    ...
    const handleSubmit =  async (event) => {
        ...
        if(Object.keys(allErrors).length === 0){
            setSubmit(true)
            setErrors({})
            const response = await postUserInDatabase()
    ...
    return (
        <section className="bg-orange-50 flex flex-col justify-center items-center h-screen">
            <form onSubmit={handleSubmit} className="p-4 bg-stone-50 rounded-md shadow-md text-center w-80">
